...
void main() {
  ...
  test(
    'salary is stored as recurring_salary, not salary_amount or a transaction',
    () async {
      ...
      expect(
        (await repo.saveSalary(const MonthlySalary(amount: 18500000))).isOk,
        isTrue,
      );
      expect(((await repo.getSalary()) as Ok).value.amount, 18500000);
      expect(
        (await repo.saveSalary(const MonthlySalary(amount: 20000000))).isOk,
        isTrue,
      );
      ...
      expect(
        (await repo.saveSalary(const MonthlySalary(amount: 21000000))).isOk,
        isTrue,
      );
      final afterSecondSave = await database.raw.query(
        recurringTransactionsTable,
        where: 'id = ?',
        whereArgs: [recurringSalaryId],
      );
      expect(afterSecondSave, hasLength(1));
      expect(afterSecondSave.single['amount'], 21000000);
    },
  );
}
