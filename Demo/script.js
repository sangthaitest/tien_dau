...
function renderTxItem(tx, { swipeable = false } = {}) {
  ...
  const inner = `
    <div class="tx-item ${swipeable ? 'swipeable' : ''}" data-id="${tx.id}" role="button" tabindex="0">
      ...
      <div class="tx-amount ${tx.type}">${sign}${formatVND(tx.amount)}</div>
    </div>
  `;
  ...
  return `
    <div class="budget-item">
      <div class="budget-item-top">
        <div class="budget-item-left">
          ...
          <div>
            ...
            <div class="tiny">${pct}% đã dùng</div>
          </div>
        </div>
        <div class="amount" style="font-size:13px">${formatVND(b.spent, true)}</div>
      </div>
      ...
      <div class="budget-meta">
        ...
        <span>Giới hạn ${formatVND(b.limit, true)}</span>
      </div>
    </div>
  `;
}
...
function animateProgressBars() {
  ...
  if (totalBar) {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => { totalBar.style.width = '64.5%'; });
    });
  }
}
...
function renderHome() {
  ...
  $('#home-budget-list').innerHTML = topBudgets.map(b => renderBudgetItem(b)).join('');
  ...
  $('#financial-tip').textContent = TIPS[Math.floor(Math.random() * TIPS.length)];
  initRipples();
}
...
function updateBalanceDisplay() {
  ...
  if (state.balanceHidden) {
    ...
    el.textContent = formatVND(state.balanceValue);
...
function renderTransactions() {
  ...
  if (sumIn) sumIn.textContent = '+' + formatVND(incomeSum, true);
  ...
  container.innerHTML = groups.map(([date, txs]) => {
    ...
    const dayLabel = dayNet >= 0 ? '+' + formatVND(dayNet, true) : '−' + formatVND(Math.abs(dayNet), true);
    return `
...
function renderCategoryGrid() {
  const expenseCats = ['food', 'coffee', 'transport', 'shopping', 'home', 'utilities', 'entertainment', 'health'];
...
function bindEvents() {
  ...
  $('#btn-hide-balance')?.addEventListener('click', () => {
    ...
    btn.setAttribute('aria-label', state.balanceHidden ? 'Hiện số dư' : 'Ẩn số dư');
    if (icon) icon.textContent = state.balanceHidden ? 'visibility_off' : 'visibility';
    // Also mask stat pills
    if (state.balanceHidden) {
      $('#monthly-income').textContent = '••••';
      $('#monthly-expense').textContent = '••••';
      $('#monthly-savings').textContent = '••••';
    } else {
      $('#monthly-income').textContent = '+18.5tr';
      $('#monthly-expense').textContent = '−8.2tr';
      $('#monthly-savings').textContent = '+10.3tr';
    }
    updateBalanceDisplay();
  });
